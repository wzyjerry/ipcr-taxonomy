...
def output_json(node):
    ...
    if 'children' in node:
        ...
        for child in node['children']:
            result = output_json(child)
            if result != {}:
                this['children'].append(result)
...
if __name__ == "__main__":
    ...
    for i in range(8):
        ...
        with open('data/json/%s.json' % chr(65 + i), 'w', encoding='utf-8') as fout:
            json.dump(output_json(ipcr), fout, ensure_ascii=False)
